...
define(['dojo/_base/declare',
  ...
  function (declare, _WidgetsInTemplateMixin, BaseWidget, TabContainer, List, IdentifyTask, IdentifyParameters,
    ...
    WidgetManager, PanelManager, LayerInfos, topic) {
    return declare([BaseWidget, _WidgetsInTemplateMixin], { /*jshint unused: false*/
      ...
      postCreate: function () {
        ...
        html.addClass(this.list.domNode, "identify-list");
        ...
        this.autoCloseNum = parseInt(this.config.infoautoclosemilliseconds, 10);
      ...
      _initSelectionGL: function() {
        ...
        this.selectionGL.name = "Identify Results Selection";
...
//       console.info(this.resultFormatString );
      },
      ...
      _bindEvents: function() {
        ...
        this.own(on(this.btnClearSel, 'click', lang.hitch(this, this._clearSel)));
        html.setStyle(this.btnClear, 'display', 'none');
        html.setStyle(this.btnClearSel, 'display', 'none');
      ...
      _clear: function () {
        ...
        if(this.extChangeHandler){
          this.extChangeHandler.remove();
        }
        return false;
      },
        ...
        if(this.list.items.length > 1){
          ...
          if (item.graphic.geometry.type === "point"){
            ...
            if(selGra.symbol.hasOwnProperty('outline')){
              ...
              selGra.symbol.outline.setWidth(4);
            }else{
              selGra.symbol.setColor(new Color([0, 255, 255, 1]));
              selGra.symbol.setWidth(4);
            }
          }
          this.selectionGL.clear();
          this.selectionGL.add(selGra);
          html.setStyle(this.btnClearSel, 'display', 'block');
        }
        var idResult = item;
      ...
      showInfoWin: function (idResult) {
        if (this.map.infoWindow) {
          ...
          if(this.config.fixedPosition){
            require(["esri/geometry/ScreenPoint", "dojo/query"], lang.hitch(this, function(ScreenPoint, query) {
              //make it so fixed position is from the right of screen
              var mWid = this.map.width;
              var fPos = this.config.fixedPosition.split(",");
              fPos[0] = mWid - (parseInt(fPos[0]) + 14); //adjust for popup offset
              fPos[1] = parseInt(fPos[1]) - 17; //adjust for popup offset
              var scrnPnt = this.map.toMap(new ScreenPoint(fPos))
              this.map.infoWindow.show(scrnPnt);
              var popupArrow = query('.esriPopup .pointer')[0];
              html.setStyle(popupArrow, 'display', 'none');
              if(this.extChangeHandler){
                this.extChangeHandler.remove();
              }
              this.own(this.extChangeHandler = this.map.on('extent-change', lang.hitch(this, function(evt){
            ...
            this.map.infoWindow.show(idResult.point);
          }
        }
      },
          ...
          if(layer.type && layer.type === 'Feature Layer'){
            ...
            if(this.config.layers.onlythese === true){
              if(this.isService2beIdentified(layer)){
                ...
                if(layer.version >= 10){
                  if((typeof layer.capabilities) !== 'string'){
                    return false;
                  }
...
//RJS 02/17/17 - Use the WAB layerInfo class to see if the layer/sublayer is visible and visible at the maps scale
              ...
              li.traversal(lang.hitch(this, function(layerInfo) {
                if(layerInfo.isShowInMap()){
                  if(layerInfo.isInScale()){
                    if(layerInfo.isLeaf() && layerInfo.isRootLayer() === false){
                      var sArr = layerInfo.id.split("_");
                      vsl.push(sArr[sArr.length - 1]);
                    }
                  }
                }
              }));
...
//End of change
      ...
      showIdentifyResults: function (r, itasks, Ids) {
        this.numServicesIdent--;
...
//            console.info(content, rsltContent);
...
//              console.info(content);
        ...
        this.iResultLen = this.list.items.length; //10/3/19 use to be +=
...
//              console.info(content, rsltContent);
              ...
              idResult.links = lyrIdLinks;
              idResult.alt = (this.list.items.length % 2 === 0);
              idResult.sym = this.resultSym;
      ...
      getResultByGID: function (gid) {
        ...
        for (var i = 0; i < this.identifyResultsArray.length; i++){
          ...
          if(parseInt(sr.id.replace('id_', ''), 10) === gid){
            retResult = sr;
            break;
          }
        }
